...
<div>
    <sa-panel>
        <el-form ref="form" class="form-query" :inline="true" :model="formQuery" label-width="100px">
            <el-form-item label="物流单号">
                <el-input placeholder="请输入" v-model="formQuery.expressNo"></el-input>
            </el-form-item>
            <!--
            <el-form-item label="物流状态">
                <el-select filterable placeholder="请选择" v-model="formQuery.status" clearable>
                    <el-option v-for="item in deliverStatusOptions" :key="item.id" :label="item.text" :value="item.id"></el-option>
                </el-select>
            </el-form-item>
            -->
        </el-form>
    ...
    <sa-panel>
        <el-table border :data="tableData" stripe style="width: 100%">
            ...
            <el-table-column prop="permissionName" label="权限名" width="200">
            </el-table-column>
            <el-table-column prop="permissionRoute" label="路由" width="200">
            </el-table-column>
...
export default {
    ...
    methods: {
        //新增
        onAdd(data){
            this.$refs.permissionForm.add(data);
        },
        //编辑
        onEdit(data){
            this.$refs.permissionForm.edit(data);
        },
        //
        onForbid(data) {
            this.$confirm('此操作将禁用该权限信息, 是否继续?', '提示', {
                type: "warning"
            }).then(() => {
                this.forbidPermission(data);
                console.log("禁用成功")
            }).catch(() => {
                this.$message({ 
                    type: 'info',
